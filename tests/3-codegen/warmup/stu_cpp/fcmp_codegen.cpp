...
void translate_main(CodeGen *codegen) {
    ...
    codegen->append_inst("bceqz   $fcc0, L1  ");
    codegen->append_inst("addi.w $t2, $zero, 1");
    codegen->append_inst("b L2");
        codegen->append_inst("L1", ASMInstruction::Label); // main 函数标签
codegen->append_inst("addi.w $t2, $zero, 0");
codegen->append_inst("L2", ASMInstruction::Label); // main 函数标签
    codegen->append_inst("st.b $t2, $sp, -17");
    ...
    {"$t0", "$fp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("xor $t0, $t0, $zero");
    codegen->append_inst("st.w",
