...
void translate_main(CodeGen *codegen) {
    ...
    {"$t2", "$sp", std::to_string(offset_map["%op0"])});
    ...
    {"$t3", "$sp", std::to_string(offset_map["%op0"])});
    codegen->append_inst("st.w",
    {"$t3","$sp", std::to_string(offset_map["%op1"])});
    ...
    {"$t1","$sp", std::to_string(offset_map["%op1"])});
    codegen->append_inst("xor $t0, $zero, $t1");
    codegen->append_inst("st.w ",
   {"$t0","$sp",std::to_string(offset_map["%op2"])  } );
