...
void CodeGen::gen_epilogue()
{
    ...
    // 在label前面加上函数名作为前缀
    append_inst(context.func->get_name() + "_ret", ASMInstruction::Label);
    // append_inst("main_ret", ASMInstruction::Label);
    //  寄存器恢复及栈帧恢复
    if (IS_IMM_12(-static_cast<int>(context.frame_size)))
    {
...
void CodeGen::gen_ret()
{
    ...
    // 存在部分 void main() {...} 的样例，其返回值是未定义的，我们要求这样的主函数通过寄存器 $a0 返回 0
    ...
    //  load_to_greg(context.inst->get_operand(0), Reg::a(0));
    // 找到函数名
    std::string func_name = context.func->get_name();
    // 在label前面加上函数名作为前缀
    append_inst("b " + func_name + "_ret");
}
...
void CodeGen::gen_br()
{
    ...
    {
        // TODO 补全条件跳转的情况
        if (branchInst->get_operand(0)->get_name() != "")
        {
            // 先把op装到寄存器
            load_to_greg(branchInst->get_operand(0), Reg::t(0));
            append_inst("bnez " + Reg::t(0).print() + ", " +
                        label_name(static_cast<BasicBlock *>(branchInst->get_operand(1))));
            append_inst("b " + label_name(static_cast<BasicBlock *>(branchInst->get_operand(2))));
        }
        else
        {
            load_to_greg(branchInst->get_operand(0), Reg::t(0));
            append_inst("bnez " + Reg::t(0).print() + ", " +
                        label_name(static_cast<BasicBlock *>(branchInst->get_operand(2))));
            append_inst("b " + label_name(static_cast<BasicBlock *>(branchInst->get_operand(1))));
        }
    }
    else
    {
...
void CodeGen::gen_icmp()
{
    ...
    std::string true_label = "true_label" + std::to_string(label_num);
    std::string false_label = "false_label" + std::to_string(label_num);
    // TODO 处理各种整数比较的情况
    switch (context.inst->get_instr_type())
    {
    case Instruction::ge:
        // 如果t0大于等于t1，t2=1，否则t2=0

        load_to_greg(context.inst->get_operand(0), Reg::t(0));
        load_to_greg(context.inst->get_operand(1), Reg::t(1));

        append_inst("bge  $t0, $t1, " + true_label);
        append_inst("addi.d $t2, $zero, 0");
        append_inst("b " + false_label);
        append_inst(true_label, ASMInstruction::Label);
        append_inst("addi.d $t2, $zero, 1");
    ...
    case Instruction::le:
        load_to_greg(context.inst->get_operand(0), Reg::t(0));
        load_to_greg(context.inst->get_operand(1), Reg::t(1));
        append_inst("bge  $t1, $t0, " + true_label);
        append_inst("addi.d $t2, $zero, 0");
        append_inst("b " + false_label);
        append_inst(true_label, ASMInstruction::Label);
        append_inst("addi.d $t2, $zero, 1");
    ...
    case Instruction::eq:
        load_to_greg(context.inst->get_operand(0), Reg::t(0));
        load_to_greg(context.inst->get_operand(1), Reg::t(1));
        append_inst("beq  $t0, $t1, " + true_label);
        append_inst("addi.d $t2, $zero, 0");
        append_inst("b " + false_label);
        append_inst(true_label, ASMInstruction::Label);
        append_inst("addi.d $t2, $zero, 1");
    ...
    case Instruction::ne:
        load_to_greg(context.inst->get_operand(0), Reg::t(0));
        load_to_greg(context.inst->get_operand(1), Reg::t(1));
        append_inst("bne  $t0, $t1, " + true_label);
        append_inst("addi.d $t2, $zero, 0");
        append_inst("b " + false_label);
        append_inst(true_label, ASMInstruction::Label);
        append_inst("addi.d $t2, $zero, 1");
...
void CodeGen::gen_call()
{
    ...
    for (auto i = 1; i < context.inst->get_num_operand(); ++i)
    {
...
void CodeGen::gen_sitofp()
{
    // TODO 整数转向浮点数
    // ffint.s.w $fd, $fj
    // 行为：选择浮点寄存器 $fj 中的整数型定点数转换为单精度浮点数，得到的单精度浮点数写入到浮点寄存器 $fd 中
    load_to_greg(context.inst->get_operand(0), Reg::t(0));
...
void CodeGen::gen_fptosi()
{
    ...
    // ftintrz.w.s $fd, $fj
    // 行为：选择浮点寄存器 $fj 中的单精度浮点数转换为整数型定点数，得到的整数型定点数写入到浮点寄存器 $fd 中
    load_to_freg(context.inst->get_operand(0), FReg::ft(0));
...
void CodeGen::run()
{
    ...
    // 先遍历所有基本块找到phi指令，把phi指令的信息存起来
    //   %op1 = phi i32 [ 0, %label_entry ], [ %op6, %label5 ],[ %op7, %label4 ]
    // 建一个vector存对应关系
        ...
        for (auto &bb : func.get_basic_blocks())
        {
            for (auto &instr : bb.get_instructions())
            {
                ...
                if (instr.get_instr_type() == Instruction::phi)
                {
                    ...
                    Value *phi_name = context.inst;
                    std::vector<std::pair<Value *, std::string>> phi_pair;
                    for (int i = 0; i < phi_inst->get_num_operand(); i += 2)
                    {
                        Value *op_name = phi_inst->get_operand(i);
                        Value *label_name = phi_inst->get_operand(i + 1);
                        auto m = (std::make_pair(phi_name, op_name));
                     //   phi_vector.insert(std::make_pair(label_name, m));
                        phi_vector.push_back(std::make_pair(label_name, m));
                    }
                }
            }
        }
            ...
            for (auto &bb : func.get_basic_blocks())
            {
                ...
                for (auto &instr : bb.get_instructions())
                {
                    ...
                    case Instruction::ret:
                        ...
                        break;
                    case Instruction::br:
                        // 处理phi指令，先判断这个基本块在哪些phi指令中是作为label出现的，然后把对应的值赋给这个phi指令前面的变量
                        //   %op1 = phi i32 [ 0, %label_entry ], [ %op6, %label5 ],[ %op7, %label4 ]
                        // 比如说这个基本块是label5，那么就把op6的值赋给op1
                        // 遍历philist，list里面每项是phi_name, phi_pair
                        // phi_pair里面每项是op_name, label_name
                        // 如果label_name是当前基本块的名字，就把op_name的值赋给phi_name

                       // if (phi_vector.find(&bb) != phi_vector.end())
                       for (auto &phi : phi_vector)
                        {
                            if (phi.first == &bb)
                            {
                                auto phi_name = phi.second.first;
                                auto op_name = phi.second.second;
                                load_to_greg(op_name, Reg::t(0));
